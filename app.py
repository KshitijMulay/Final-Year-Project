...
def correct_with_gemini(statement):
    # URL and API key placeholder for Gemini API
    api_key = "AIzaSyDbooV7wkjUrULchHBDXETC-crLt1YynIg"

    chat = model.start_chat(history=[])
    
    system_prompt = "You are a kind helpful assistant. Please evaluate the user's input."
    user_input = f"User input: {statement}"
    evaluation_prompt = "Evaluate the input and give me improvement Suggestion only for Incorrect part in brief. don't ask for feedback"

    response = chat.send_message([system_prompt, user_input, evaluation_prompt])
...
def process_audio():
    ...
    try:
        ...
        statement = audio_text.lower()
        audio_text = preprocess_text(audio_text)
        

        print(f"Recognized Audio Text: {statement}")
        ...
        improvement_suggestions = generate_improvement_suggestions(accuracy, audio_keywords, dataset_keywords, statement)
        ...
        return jsonify({
            ...
            "dataset_content": dataset_content[:800],
...
def generate_improvement_suggestions(accuracy, audio_keywords, dataset_keywords, statement):
    suggestions = []
    if accuracy < 50:
    ...
    if len(set(audio_keywords) & set(dataset_keywords)) < len(dataset_keywords) / 2:
        suggestions.append("Consider mentioning more keywords related to topic :" + ", ".join(set(audio_keywords)))
    if accuracy > 80:
        suggestions.append("Great job! Try to maintain this level of accuracy.")
    corrected_statement = correct_with_gemini(statement)
    if corrected_statement:
        suggestions.append("Corrected statement - " + corrected_statement)
...
#         try:
#             textt = recognizer.recognize_google(audio)
